...
export class McqComponent implements OnInit {
  ...
  quizlist() {
    ...
    this.service.getallquestions(this.qid).subscribe((res: any) => {
      ...
      console.log(this.que);
      // Object.values(this.que).map((ele, i)=>{
      //   console.log(ele,i);
      //   console.log(this.que,"queeee");
      //   console.log(this.mydata,"hhhhhhhhhhh");
  ...
  setAnswer(option: string, questionid: number, ans: string, question: any) {
    ...
    (<HTMLInputElement>(
      document.getElementById(this.qidid + '1')
    )).disabled = true;
    (<HTMLInputElement>(
      document.getElementById(this.qidid + '2')
    )).disabled = true;
    (<HTMLInputElement>(
      document.getElementById(this.qidid + '3')
    )).disabled = true;
    (<HTMLInputElement>(
      document.getElementById(this.qidid + '4')
    )).disabled = true;

    var opt1 = (<HTMLInputElement>document.getElementById(this.qidid + '11'))
      .innerText;
    var opt2 = (<HTMLInputElement>document.getElementById(this.qidid + '22'))
      .innerText;
    var opt3 = (<HTMLInputElement>document.getElementById(this.qidid + '33'))
      .innerText;
    var opt4 = (<HTMLInputElement>document.getElementById(this.qidid + '44'))
      .innerText;
    debugger;

    if (opt1 == ans) {
      (<HTMLInputElement>(
        document.getElementById(this.qidid + '11')
      )).classList.add('correct');
    } else if (opt2 == ans) {
      (<HTMLInputElement>(
        document.getElementById(this.qidid + '22')
      )).classList.add('correct');
    } else if (opt3 == ans) {
      (<HTMLInputElement>(
        document.getElementById(this.qidid + '33')
      )).classList.add('correct');
    } else if (opt4 == ans) {
      (<HTMLInputElement>(
        document.getElementById(this.qidid + '44')
      )).classList.add('correct');
    }
  ...
  onblurdesc(questionid: number, questionanswer: string) {
    let status = {
      quizeId: this.qid,
      userId: this.uid,
      userans: this.desc,
